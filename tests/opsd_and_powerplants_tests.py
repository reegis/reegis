...
class TestOpsd2reegis:
    ...
    def setUpClass(cls):
        ...
        cls.pp2["efficiency_{0}".format(year)] = cls.pp2[
            "capacity_{0}".format(year)
        ].div(cls.pp2["capacity_in_{0}".format(year)])
    ...
    def test_001(self):
        eq_(
            int(self.pp.groupby("fed_states").sum().loc["BE", "capacity"]),
            2427,
        )

        eq_(
            round(self.pp2.loc[("BE", "Hard coal"), "efficiency_2014"], 3),
            0.386,
        )
    ...
    def test_002(self):
        ...
        eq_(
            int(pp.groupby("fed_states").sum().loc["BE", "capacity_2000"]),
            2391,
        )

        eq_(
            coastdat.windzone_region_fraction(pp, name="fed_states", year=year)
            .round(2)
            .loc["NI", 3],
            0.24,
        )
